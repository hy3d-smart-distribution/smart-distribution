...
namespace Lean.Touch
{
	...
	public class HeightLeanTranslate : MonoBehaviour
	{
		...
		protected virtual void Translate(Vector2 screenDelta)
		{
			...
			if (camera != null)
			{
                ...
                Vector3 parentPos = GameObject.FindGameObjectsWithTag("upperPivot")[1].transform.position;
                // Convert back to world space
                //transform.position = camera.ScreenToWorldPoint(screenPoint);
                Vector3 move = camera.ScreenToWorldPoint(screenPoint);
                Vector3 offset = move - originPos;
                Vector3 a = move - parentPos;
                Vector3 b = originPos - parentPos;
                if (a.x > b.x)
                {

                    transform.Translate(Vector3.back * (a.x - b.x));
                }
                else
                {
                    transform.Translate(Vector3.forward * (b.x - a.x));
                }
            }
		}
	}
