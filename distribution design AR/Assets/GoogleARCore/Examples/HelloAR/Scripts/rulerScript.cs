    ...
    public class rulerScript : MonoBehaviour
    {
        ...
        private void ExecMenu(bool howDo)
        {
            if (howDo == true)
            {
                ...
                isSetRotation(setRotation);
                rotationBTN.SetActive(true);
                listInOBJ.SetActive(true);
                deleteBTN.SetActive(true);
                fullshot.SetActive(true);
            }
            else
            {
        ...
        public void WholeCtrl() {
            if (poleCount == 1 && whole == false)
            {
                whole = true;
                GameObject.Find("pole").transform.Translate(Vector3.up * 1.5f);
                //부분부분 select는 잠금
                //전체의 select는 활성화
            }
            else if (poleCount == 1 && whole == true) {
                whole = false;
                GameObject.Find("pole").transform.Translate(Vector3.down * 1.5f);
                //부분부분 select는 잠금
                //전체의 select는 활성화
            }
        }
